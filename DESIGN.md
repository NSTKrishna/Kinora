<!--
  Kinora's design system.

  Adapted from the Runway analysis in VoltAgent/awesome-design-md. Kinora is a
  dark, cinematic, media-first studio, so the atmosphere, typography, chrome and
  layout rules below are adopted as written.

  Three deliberate deviations, and why:

  1. ACCENT — Runway has no interface colour at all. Kinora keeps one: the ember
     (#fb6d2b) in its mark. It follows Runway's discipline rather than Runway's
     palette — brand mark, focus rings, the credit meter and a small number of
     primary actions. Never decorative, never a fill for its own sake. This is
     what keeps Kinora's identity its own, per AGENTS.md.

  2. TYPEFACE — abcNormal is Runway's licensed custom face. This document names
     Inter as a close substitute; Kinora already ships Inter, so Inter carries
     every role. The single-typeface commitment is the rule that matters, not
     the particular face.

  3. LIGHT SECTIONS — Runway alternates dark and light. Kinora is dark-only
